...
export default class ZBridgePrefs extends ExtensionPreferences {
    fillPreferencesWindow(window) {
        ...
        frontRow.connect('notify::selected', () => {
             this._runSilentCommand(['zb-config', '-F', orientations[frontRow.get_selected()]]);
        });
        ...
        backRow.connect('notify::selected', () => {
             this._runSilentCommand(['zb-config', '-B', orientations[backRow.get_selected()]]);
        });
            ...
            try {
                ...
                savedIps.forEach(ip => {
                    ...
                    connBtn.connect('clicked', () => {
                        this._runCommand(['zb-config', '-i', ip], connBtn, null);
                    });
                    ...
                    delBtn.connect('clicked', () => {
                        // Remove via Backend
                        this._runSilentCommand(['zb-config', '-R', ip]);
        ...
        try {
            ...
            proc.wait_check_async(null, (proc, res) => {
                try {
                    ...
                    if (saveIp && ipStr) {
                        this._runSilentCommand(['zb-config', '-A', ipStr]);
                        setTimeout(() => this._refreshSavedList(), 500);
                    }
                } catch (e) {
